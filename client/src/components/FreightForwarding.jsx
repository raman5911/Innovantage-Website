...
class FreightForwarding extends React.Component {
	...
	// countRecord() {
	// 	fetch("/api/count/freight_forwarding")
	// 		.then((res) => res.json())
	// 		.then((result) => {
	// 			this.setState({
	// 				totalCount: result,
	...
	fetchData() {
		...
		// fetch("/api/freight_forwarding/" + this.state.currentPage)
		// 	.then((res) => res.json())
		// 	.then((result) => {
		// 		console.log(result);

		// 		// var newCount = this.state.totalCount - result.length;
	...
	goToView(user) {
		this.props.history.push("/dashboard/showFreightData", { id: user._id });
	}
		...
		if (!this.state.isLoaded) {
			...
			return (
				<div className="container">
					...
					<h3 style={{ marginTop: "0.5rem" }}>Freight Forwarding</h3>
...
export default withRouter(FreightForwarding);

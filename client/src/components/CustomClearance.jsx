...
class CustomClearance extends React.Component {
	...
	goToView(user) {
		this.props.history.push("/dashboard/showCustomData", { id: user._id });
	}

	editData(user) {
		this.props.history.push("/dashboard/editCustomData", {
			userData: user,
		});
		this.fetchData();
	}
...
export default withRouter(CustomClearance);
